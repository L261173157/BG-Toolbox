...
class MaterialClassifier:
        ...
        try:
            ...
            for rule in classification_rules:
                ...
                if len(parts) == 2:
                    ...
                    normalized_main = (
                        main_category.strip().lower().replace(" ", "").replace("\t", "")
                    )
                    normalized_sub = (
                        sub_category.strip().lower().replace(" ", "").replace("\t", "")
                    )
    ...
    def _call_deepseek_api(self, prompt):
        ...
        # 定义 web_search 工具
        # 根据配置决定是否启用 web_search 工具。
        # 说明：默认情况下该功能为禁用（减少延迟和外部依赖）。
        # 如果需要更高的分类准确度，可通过环境变量启用（见 README / CONFIGURATION.md）。
        tools = []
        if getattr(Config, "ENABLE_WEB_SEARCH", False):
            tools = [
                {
        ...
        for attempt in range(Config.MAX_RETRIES):
            try:
                # 使用 previous_response_id 维持对话上下文
                response = self.client.responses.create(
                    model=self.model,
                    previous_response_id=self.conversation_context_id,
                    input=[
                        {"role": "user", "content": prompt},
                    ],
                    temperature=0.1,  # 降低随机性，提高稳定性
                    tools=tools,
                    extra_body=extra_body  # 禁用thinking
                )
    ...
    def validate_classification_result(self, result):
        ...
        try:
            ...
            original_main, original_sub = self.classification_mapping[
