...
class MaterialManager:
    ...
    def process_material(self, material_data):
        ...
        try:
            ...
            try:
                json_str = json.dumps(material_data, default=json_default, ensure_ascii=False)
            except Exception as json_e:
                logger.warning(f"JSON序列化失败，使用字符串表示: {json_e}")
                json_str = str(material_data)
            logger.error(f"物料处理失败: {json_str} -> {e}")
            return {
                "original_data": material_data,
                "error": str(e),
                "status": "failed"
    ...
    def process_batch(self, materials_list, max_workers=5, max_samples=None):
        ...
        if not processed_materials:
            logger.info("没有需要处理的物料")
            return []

        results = []
        lock = threading.Lock()
        processed_count = 0  # 已处理数量
        progress_interval = max(1, total_materials // 10)  # 进度记录间隔
        ...
        def thread_process(material_data):
            ...
            try:
                ...
                # 处理线程中的异常
                logger.error(f"线程任务处理失败: {material_data.get('物料名称', '未知物料')} -> {e}")
                result = {
                    "original_data": material_data,
                    "error": str(e),
                    "status": "failed"
            ...
            with lock:
                ...
                processed_count += 1
                # 每处理指定数量或最后一个物料时记录进度
                if processed_count % progress_interval == 0 or processed_count == total_materials:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            # 提交所有任务
            futures = list(executor.map(thread_process, processed_materials))
            
            # 由于process_material内部已经有API调用限制，这里不再需要额外的sleep
            # 每个线程在调用API时会自动处理速率限制

        logger.info(f"批量处理完成，共处理 {total_materials} 条物料")
        return results
    ...
    def read_materials_from_csv(self, csv_file_path):
        ...
        try:
            ...
            # 提取需要的列，不存在的列会被自动忽略
            required_columns = ["物料名称", "图号/型号", "材料", "分类/品牌"]
